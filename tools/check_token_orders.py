...
@mcp.tool()
async def check_token_orders(chain_id: str, token_address: str):
    """
    ...
    Raises:
        ValueError: If chain_id or token_address is empty
    """
    if not chain_id or not token_address:
        raise ValueError("chain_id and token_address must be non-empty strings")

    data = await make_request(f"/orders/v1/{chain_id}/{token_address}")
